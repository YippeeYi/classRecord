(() => {
    ...
    function rollRarity(state, guaranteeFourStar = false) {
        ...
        if (guaranteeFourStar || state.pity4 >= 9) {
            return Math.random() < 0.12 ? 5 : 4;
        }
    ...
    function draw(count) {
        ...
        const pity = {
            pity4: state.gacha.pity4 || 0,
            pity5: state.gacha.pity5 || 0
        };
        const pool = groupedPool();
        const results = [];

        for (let index = 0; index < count; index += 1) {
            const rarity = rollRarity(pity, count === 10 && index === count - 1 && !results.some((item) => item.rarity >= 4));
            ...
            return `
                <article class="wish-card ${rarityClass(item.rarity)}" style="animation-delay:${index * 90}ms">
                    ...
                    <div class="wish-card-meta">
                        ...
                        <p>${escapeHtml(item.title || item.series || '等待替换你的卡面素材')}</p>
                    </div>
                </article>
            `;
        }).join('');
